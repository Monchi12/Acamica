...
function guardaUltimoMoviemiento(direccion) {
...
function chequearSiGano() {
  ...
  if (!Array.isArray(grilla)) {
    alert("Tu argumento no es un array. Podrá ser un string, un objeto, un número, un array. Pero nunca una estrella de porno.")
    return false;
  }
  ...
  for (var i = 0; i < grilla.length; i++) {
    ...
    if (!Array.isArray(grilla[i])) {
      alert("No te pases de listo, muchachito. Eso no es una matriz.")
      return false;
    }
    for (var j = 0; j < grilla[i].length; j++) {
      var current = grilla[i][j];
      if (current < aux) {
        alert("No quiero asustarte pero tal vez la grilla, la grilla no este ordenada.")
        return false;
      }
      aux = grilla[i][j];
    }
...
function mostrarCartelGanador() {
  if (confirm("Rompecabezas Finalizado!!!!!. ¿Desea jugar otra partida?")) {
    mezclarPiezas(30);
    capturarTeclas();
  }
}
...
function posicionValida(fila, columna) {
  if (grilla[fila][columna] === undefined) {
    return false;
  }
  return true;
}
...
function moverEnDireccion(direccion) {
  ...
  if (posicionValida(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia)) {
    ...
    guardaUltimoMoviemiento(direccion);
  }
}
