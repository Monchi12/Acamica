direccionesModulo = (function () {
  ...
  function agregarDireccionEnLista(direccion, coord) {
    ...
    for (i = 0; i < pIntermedios.length; ++i) {
      if (pIntermedios.options[i].text.replace(/\r?\n|\r/g, ' ') === direccion.replace(/\r?\n|\r/g, ' ')) {
  ...
  function calcularYMostrarRutas() {
    ...
    const pIntermedios = document.getElementById('puntosIntermedios');
    let travelMode = '';
    const waypoint = [];
    ...
    for (let i = 0; i < pIntermedios.length; i++) {
      if (pIntermedios[i].selected) {
        waypoint.push({
          location: pIntermedios[i].value,
          stopover: true
        });
      };
    };
